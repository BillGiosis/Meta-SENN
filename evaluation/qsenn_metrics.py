...
def evaluateALLMetricsForComps(features_train,  outputs_train,  feature_maps_test,
    ...
    with torch.no_grad():
        ...
        batch_size = 300
        num_batches = int(np.ceil(len(outputs_test) / batch_size))
        for i in range(num_batches):
            start_idx = i * batch_size
            end_idx = min((i + 1) * batch_size, len(outputs_test))
            localizer(outputs_test[start_idx:end_idx].to("cuda"),
                    feature_maps_test[start_idx:end_idx].to("cuda"))
