...
def main(dataset, arch, seed=None, model_type="qsenn", do_dense=True, crop = True, n_features = 50, n_per_class=5, img_size=448, reduced_strides=False, meta_rl=True, meta_iterations=10, device='cuda'):
    ...
    if model_type == "metasenn":    #Extra variable for enabling meta reinforcement learning
        do_meta_rl = True
        model_type = "qsenn"
    ...
    final_model = finetune(model_type, model, train_loader, test_loader, log_dir, n_classes, seed, architecture_params[arch]["beta"],
        OptimizationSchedule, n_per_class, n_features, do_meta_rl, meta_iterations, device)
    ...
    json_save(os.path.join(log_dir, f"Results_{model_type}_{n_features}_{n_per_class}_{time}_FinetunedModel.json"), metrics_finetuned)
    print("Done")
    pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="FGVCAircraft", type=str, help='dataset name', choices=["CUB2011", "FGVCAircraft", "TravelingBirds", "StanfordCars"])
    parser.add_argument('--arch', default="resnet50", type=str, help='Backbone Feature Extractor', choices=["resnet50", "resnet18"])
    parser.add_argument('--model_type', default="metasenn", type=str, help='Type of Model', choices=["metasenn", "qsenn", "sldd"])
    ...
    parser.add_argument('--meta_iterations', default=100, type=int, help='Number of meta-learning iterations')
    parser.add_argument('--device', default='cuda', type=str, choices=['cuda', 'cpu'], help='Device to run meta-RL training on')

    args = parser.parse_args()
    main(args.dataset, args.arch, args.seed, args.model_type, args.do_dense,args.cropGT, args.n_features, args.n_per_class, args.img_size, args.reduced_strides, True, args.meta_iterations, args.device)
